...
(defun clime-ac-delete-text-back-to-call-target ()
...
target of the call. Point should be be over last character of call target."
  (let ((p (point)))
    ...
    (let ((text (buffer-substring-no-properties (point) p))
	  (deactivate-mark nil))
...
Bind clime-buffer-connection and buffer-file-name to the given values.
...
changes will be forgotten."
  `(let ((buf (current-buffer))
	 ...
	 (p (point))
	 (conn (clime-current-connection)))

     (unwind-protect
	 (with-temp-buffer
	   (let ((clime-buffer-connection conn)
       ...
       (clime-write-buffer nil t)
       )))


(defun clime-ac-trunc-summary (str)
  (let ((len (length str)))
    (if (> len 40)
	(concat (substring str 0 40) "...")
      str)))

(defun clime-ac-candidate-name (c)
  (get-text-property 0 'symbol-name c))

(defun clime-ac-candidate-type-sig (c)
  (get-text-property 0 'type-sig c))

(defun clime-ac-get-doc (item)
  "Return doc for given item."
  (get-text-property 0 'type-sig item))

(defun clime-pt-at-end-of-prev-line ()
...
(defun clime-ac-get-active-param-info ()
  ...
  (save-excursion
	...
	(while (> (point) lbound)
	  (cond
	   ((clime-in-string-or-comment (point)) nil)
...
(defun clime-ac-update-param-help ()
  "When entering the arguments to a call, display a tooltip
   with the param names and types of the call."
  (let ((info (clime-ac-get-active-param-info)))
    (if info
	(let* (;; To be used for tooltip positioning..
	       (name-end (plist-get info :name-end-point))
	       (call-info (plist-get info :call-info))
	       (signature (clime-ac-call-info-signature call-info)))
	  (message signature))
      (remove-hook 'post-command-hook 'clime-ac-update-param-help t))))


(defun clime-ac-call-info-signature (call-info)
	...
	(let ((params (plist-get sect :params))
	  ...
	  (propertize (concat "("
				 ...
				 (format
				  ...
				  (propertize (clime-type-name-with-args
					       (cadr nm-and-tp))
     ...
     " => "
     (propertize
      (clime-type-name-with-args result-type)
      'face font-lock-type-face)
     )))


(ac-define-source clime-members
  '((document . clime-ac-get-doc)
    (candidates . (clime-ac-member-candidates ac-prefix))
    (prefix . clime-ac-member-prefix)
...
(defun clime-ac-disable ()
  (auto-complete-mode 0)
  )

(provide 'clime-auto-complete)